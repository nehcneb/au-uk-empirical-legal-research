...
# %%
from functions.austlii_functions import austlii_methods_dict, austlii_sort_dict, austlii_highlight_dict, austlii_databases_default_dict, austlii_databases_dict, austlii_search_tool, austlii_search_url


# %%
#function to create dataframe
def austlii_create_df():
    ...
    #Databases
    databases_list = databases_entry
...
if 'df_master' not in st.session_state:
    ...
    'Enter your search': None,
    'From date': None,
    'To date': None,
    'Sort results by': list(austlii_sort_dict.keys())[0],
...
if 'jurisdiction_page' not in st.session_state:
    st.session_state['jurisdiction_page'] = 'pages/AUSTLII.py'
...
# %%
#if st.session_state.page_from != "pages/AUSTLII.py": #Need to add in order to avoid GPT page from showing form of previous page

#Create form for court selection

return_button = st.button('RETURN to first page')

st.header(f"Search :blue[AustLII]")
...
st.subheader("Select the database(s) to search")

default_on = st.button(f"SELECT 'All Case Law Databases'", help = 'You may need to press :red[RESET] to add these databases.')

if default_on:
    
    st.session_state['df_master']['Databases'] = st.session_state['df_master']['Databases'].astype('object')
    st.session_state['df_master'].at[0, 'Databases'] = list(austlii_databases_default_dict.keys())[0]

databases_entry = st.multiselect(label = 'Select or type in the databases to search', options = list(austlii_databases_dict.keys()), default = st.session_state['df_master'].loc[0, 'Databases'])

#st.caption("All databases listed in this menu will be covered if left blank.")

#Search terms

st.subheader("Your search terms")

st.markdown("""For search tips, please visit [AustLII](https://austlii.edu.au/advanced_search.shtml). This section mimics their advanced search function.
""")

method_entry = st.selectbox(label = 'Method', 
                                      options = [*austlii_methods_dict.keys()], 
                                    index = list_value_check([*austlii_methods_dict.keys()], st.session_state['df_master'].loc[0, "Method"]), 
                                    )
query_entry = st.text_input(label = 'Enter your search', value = st.session_state.df_master.loc[0, 'Enter your search'], help = 'e.g. consumer near protection')
...
sortby_entry = st.selectbox(label = 'Sort results by', 
                                      options = [*austlii_sort_dict.keys()], 
                                    index = list_value_check([*austlii_sort_dict.keys()], st.session_state['df_master'].loc[0, "Sort results by"]), 
                                    )
...
highlight_entry = True #Can't unselect this
...
if preview_button:
    ...
    all_search_terms = str(query_entry) + str(from_date_entry) + str(to_date_entry)
    
    if all_search_terms.replace('None', '') == "":

        st.warning('You must enter some search terms.')

    elif len(databases_entry) == 0:
        
        st.warning('Please select at least one database to cover.')
        ...
        with st.spinner(r"$\textsf{\normalsize Getting your search results...}$"):
            ...
            try:
                ...
                df_master = austlii_create_df()
                
                search_results_w_count = austlii_search_url(df_master)
...
if return_button:
    ...
    st.session_state["page_from"] = 'pages/AUSTLII.py'
