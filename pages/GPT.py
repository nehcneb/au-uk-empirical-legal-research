...
print(f"The pause between document scraping is {scraper_pause_mean} second.\n")

print(f"The lower bound on lenth of document text to process is {judgment_text_lower_bound} tokens.\n")
...
st.header(":blue[Would you to ask GPT questions about the documents returned by your search terms?]")

st.markdown("""You can use this app without asking GPT any questions. For instance, you can extract metadata and get estimates of document length and GPT cost. 
""")
...
st.caption("Use of GPT is costly and funded by a grant. For the model used by default (gpt-4o-mini), Ben's own experience suggests that it costs approximately USD \$0.01 (excl GST) per document. The [exact cost](https://openai.com/pricing) depends on question length, document length, and answer length. You will be given cost estimates.")

st.subheader("Enter your questions for each document")

st.markdown("""Please enter one question **per line or per paragraph**. GPT will answer your questions for **each** document based only on information from **that** document. """)

st.markdown("""GPT is instructed to avoid giving answers which cannot be obtained from the relevant document itself. This is to minimise the risk of giving incorrect information (ie hallucination).""")
...
st.caption(f"By default, answers to your questions will be generated by model gpt-4o-mini. Due to a technical limitation, this model will read up to approximately {round(tokens_cap('gpt-4o-mini')*3/4)} words from each document.")
...
if own_account_allowed() > 0:
    ...
    st.markdown("""Would you like to increase the quality and accuracy of answers from GPT, or change the maximum nunber of documents to process? You can do so with your own GPT account.
""")
...
""")
        ...
        st.write(f'**:green[You can change the maximum number of documents to process.]** The default maximum is {default_judgment_counter_bound}.')
        ...
        if judgments_counter_bound_entry:
            ...
            if judgments_counter_bound_entry > st.session_state["judgment_batch_cutoff"]:
                ...
                st.warning(f"Given more than {st.session_state['judgment_batch_cutoff']} documents need to be processes, this app will send your requested data to your nominated email address in about **2 business days**.")
...
st.markdown("""By using this app, you agree that the data and/or information this form provides will be temporarily stored on one or more remote servers for the purpose of producing an output containing data in relation to documents. Any such data and/or information may also be given to an artificial intelligence provider for the same purpose.""")
...
if ((own_account_allowed() > 0) and (batch_mode_allowed() > 0) and (st.session_state.jurisdiction_page in ['pages/HCA.py', 'pages/FCA.py', 'pages/NSW.py',  'pages/US.py'])):
    st.markdown(f"""Alternatively, you can press :orange[REQUEST data] to process up to {st.session_state["judgment_counter_max"]} documents. Your requested data will be sent to your nominated email address in about **2 business days**. 
""")
...
if run_button:
    ...
    if int(consent) == 0:
        ...
        #st.write('Your results should be available for download soon. The estimated waiting time is 3-5 minutes per 10 documents.')
        #st.write('If this app produces an error or an unexpected spreadsheet, please double-check your search terms and try again.')

        with st.spinner(r"$\textsf{\normalsize \textbf{In progress...} The estimated waiting time is 3-5 minutes per 10 documents.}$"):
            ...
            try:
                ...
                if len(df_individual) == 0:
                    st.error('Your search terms may not return any documents. Please return to the previous page and press the PREVIEW button to double-check.')
