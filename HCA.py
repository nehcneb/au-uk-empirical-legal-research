...
def create_df():
    ...
    new_row = {'Processed': '',
           ...
           'Your GPT API key': gpt_api_key, 
            ...
            'Search by one citation': citation, 
...
def run(df_master):
    ...
    if len(df_master.loc[0, 'Search by one citation']) > 0:
        #direct_link = mnc_to_link(df_master.loc[0, 'Collection'], df_master.loc[0, 'Search by one citation'])
        direct_link = citation_to_link(df_master.loc[0, 'Collection'], df_master.loc[0, 'Search by one citation'])
...
citation_entry = st.text_input('Search by one citation (eg [1992] HCA 23 or 175 CLR 1 but not both)')
#st.write('You can only enter one citation. Medium neutral citation is preferred. Other citations work only up to 1998.')
